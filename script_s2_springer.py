...
for art_springer in springer.iloc[1:, 0]:
    for art_atual in atual.iloc[1:, 0]:
        ...
        if (art_springer == art_atual) or (titulo_menor in art_springer):
            ...
            if atual.iloc[linha[0],colum[0]+2] == "Scopus":
                l,c = np.where(atual.values == art_atual)
                atual.iloc[l[0],c[0]+2] = "Scopus + Springer"  # Mudança
                print("Springer: ", art_springer)
                ...
                atual.iloc[l[0],c[0]+2] = "Springer"  # Mudança
...
for art_atual in atual.iloc[1:, 2]:
    if art_atual == "Springer" or art_atual == "Scopus + Springer":
        count = count+1
